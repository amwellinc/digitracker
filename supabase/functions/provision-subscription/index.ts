...
//
// Flow: validate → create the Supabase Auth user → create sub_accounts /
// users / subscriptions rows → notify Super-Admins (in-app + best-effort
// email) → for paid plans, create a Stripe Customer + Checkout Session and
// hand the URL back for the browser to redirect to. Free plans skip Stripe
// entirely and are active immediately.
//
...
Deno.serve(async (req) => {
  ...
  // Email must be globally unique — createUser fails cleanly if not, but a
  // pre-check gives a clearer message than Supabase's generic auth error.
  ...
  // ── Create the Auth user first — everything else rolls back if it fails ──
  ...
  if (authErr || !authData?.user) {
    return json({ error: authErr?.message ?? 'Failed to create your account.' }, 400)
  }
  ...
  // ── First user (Admin) for the new sub-account ───────────────────────────
  ...
  await sendNotificationEmails(admin, {
    companyName, adminName: adminName || companyName, adminEmail,
    planName: planCfg.name, superAdmins: (superAdmins ?? []) as { email: string; name: string }[],
  })
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${origin}/#/subscribe/thank-you?code=${code}&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${origin}/#/subscribe?cancelled=true`,
    })

    return json({ success: true, code, checkoutUrl: session.url })
  } catch (err) {
...
async function sendNotificationEmails(
  ...
  info: { companyName: string; adminName: string; adminEmail: string; planName: string; superAdmins: { email: string; name: string }[] },
) {
  try {
    ...
    if (!platform?.smtp_host || !platform.smtp_user || !platform.smtp_pass || !platform.from_email) return
    ...
    await client.send({
      from,
      to: info.adminEmail,
      subject: `Welcome to DIGITRACKER — ${info.planName} plan`,
      content: 'auto',
      html: `<p>Hi ${info.adminName},</p>
        <p>Thanks for signing up for DIGITRACKER on the <strong>${info.planName}</strong> plan for <strong>${info.companyName}</strong>.</p>
        <p>You can sign in any time at <a href="https://digitracker-app.digi5y.co/#/login">digitracker-app.digi5y.co</a>.</p>
        <p>— The DIGITRACKER team</p>`,
    })
