...
type Tab = 'metrics' | 'duties' | 'logs' | 'points'
...
export function KPIAdminPanel() {
  ...
  useEffect(() => {
    ...
    void supabase.from('performance_points').select('*')
      ...
      .then(({ data }) => { setMemberPerfPoints((data ?? []) as PerformancePoints[]) })
  }, [tab, selectedUserId])

  // Sync draft when selectedDate or points change
  useEffect(() => {
  ...
  function historyWeeks(): { weekStart: string; entries: PerformancePoints[]; total: number }[] {
    ...
    for (const p of memberPerfPoints) {
      ...
      const arr = map.get(ws) ?? []; arr.push(p); map.set(ws, arr)
    }
            ...
            {filteredMembers.map(m => {
              ...
              return (
                <button key={m.id} onClick={() => setSelectedUserId(m.id)}
                  ...
                  <span className={`text-sm font-medium ${active ? 'text-violet-700' : 'text-gray-700'}`}>
                    {m.name}
                  </span>
                </button>
              )
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-gray-800">Daily Checklist</p>
                <span className="text-xs text-gray-400 bg-gray-100 rounded-full px-2 py-0.5">
                  {(kpiConfig?.checklists ?? []).length} items
                </span>
              </div>
              <p className="text-[11px] text-gray-400">Staff tick these daily with optional remarks</p>
              {saving && <span className="text-xs text-violet-500 animate-pulse ml-2">Saving…</span>}
            </div>
            ...
            {loadingKpi ? (
              ...
              <div className="px-6 py-5 text-center text-gray-400">
                <p className="text-sm">No checklist items yet — add one below</p>
              </div>
            ) : (
                ...
                {(kpiConfig?.checklists ?? []).map((item, i) => (
                  <div key={i} className="flex items-center gap-3 px-5 py-2.5">
                    ...
                    <button onClick={() => void deleteCheckItem(i)}
                      className="text-gray-300 hover:text-red-500 text-sm flex-shrink-0 transition-colors">✕</button>
                  </div>
                ))}
              </div>
            )}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-gray-400">
                          {existingForDate ? 'Rating saved — edit below to update' : 'No rating yet'}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <p className="text-xs text-gray-500">
                        {draftVal < 0 ? '⚠️ Negative — staff will see this.' :
                         draftVal > 0 ? '🌟 Positive rating!' : '⊙ Neutral.'}
                      </p>
